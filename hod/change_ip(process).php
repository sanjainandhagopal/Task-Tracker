...
if (isset($_POST['login'])) {
	...
	$result = $conn -> query("SELECT * FROM hod WHERE log_id='$admin_id'") or die($mysqli -> error());
	$row = $result -> fetch_array();

	if ($o_apass==$apass) 
	{
		$conn -> query("UPDATE hod SET log_id = '$aname',pass = '$apass' WHERE log_id='$admin_id'") or die($mysqli -> error());
